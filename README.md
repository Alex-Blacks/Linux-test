...
nice - "nice -n 10 командa" - используется для установки приоритета для нового процесса.
        
        Диапазон:
                от -20 (максимальный приоритет)
                до +19 (минимальный приоритет)

renice - "renice -n 5 -p <PID>" - используется для установки приоритета для существующего процесса.
...
lsof - покажет список всех открытых файлов и связанных с ними процессов
        ...
        "sudo lsof -u alex" - Только файлы определённого пользователя
        "sudo lsof /var/log/syslog" - Кто использует конкретный файл
        "sudo lsof -i :80" - Кто слушает порты (например, 80)
...
ss - "sudo ss -tulpn" - Какие процессы слушают порты

        -t - TCP
        -u - UDP
        -l - listening (слушающие)
        -n - numeric (показать цифры вместо имён)
